...
I=function(x,y,base=2){
  ...
  Hx=-sum(px*log(px,base))
  return((Hy-Hyx)/Hx)
}

#定义C4.5树节点形式(C4.5决策树不一定为二叉树，每一节点的子节点个数等于所选特征xi的取值数)
...
#2.特征集为空集
#3.节点的最大信息增益小于阈值(eps)
c4.5=function(x,y,eps=1e-6,base=2,cut=FALSE,alpha=NULL){
  n=nrow(x)
  p=ncol(x)
  tree=list()
  currentnode=1
  ...
  while (TRUE){
    ...
    #若当前节点的的x所对应的y均属于同一类C或当特征集为空集，则将该节点修改为叶节点
  ...
  if (cut==FALSE) return(tree) else{
    return(c4.5.cut(x,y,tree,alpha,base))
  }
}

#C4.5剪枝算法
c4.5.cut=function(x,y,tree,alpha,base=2){
  lossfunction=function(y,index,alpha,base=2){
...
#C4.5预测
c4.5.predict=function(xpre,tree){

  #对单条记录进行预测
  c4.5.predictONE=function(xpre,tree){
    #从根节点开始查找ID3树
    currentnode=1
  ...
  #对xpre用apply
  if (nrow(xpre)==1){
    return(c4.5.predictONE(xpre,tree))
  }else return(apply(xpre,1,c4.5.predictONE,tree))
}
...
tree=c4.5(x,y)
c4.5.cut(x,y,tree,alpha = 0.5)
c4.5(x,y,cut = TRUE,alpha = 0.5)

xpre=x[4,]
c4.5.predict(xpre,tree)
c4.5.predict(x,tree)
tree[[2]]$son=NA
c4.5.predict(xpre,tree)
c4.5.predict(x,tree)
